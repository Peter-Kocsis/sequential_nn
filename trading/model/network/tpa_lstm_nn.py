...
class TPALSTM(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.relu = nn.ReLU()
        self.lstm = nn.LSTM(
            hidden_size, hidden_size, n_layers, bias=True, batch_first=True
        )  # output (batch_size, obs_len, hidden_size)
        self.hidden_size = hidden_size
        self.filter_num = num_filters
        self.filter_size = hidden_size
        self.output_sequence_length = output_sequence_length
        self.attention = TemporalPatternAttention(
            self.filter_size, self.filter_num, input_sequence_length - 1, hidden_size
        )
        self.linear = nn.Linear(hidden_size, output_sequence_length)
        self.n_layers = n_layers

    def forward(self, x):
        batch_size, obs_len, feature_size = x.size()
        x = x.view(batch_size, obs_len, feature_size)
        xconcat = self.relu(self.hidden(x))
        # x = xconcat[:, :obs_len, :]
        # xf = xconcat[:, obs_len:, :]
        H = torch.zeros(batch_size, obs_len - 1, self.hidden_size, device=x.device)
        ht = torch.zeros(self.n_layers, batch_size, self.hidden_size, device=x.device)
        ct = ht.clone()
...
class TemporalPatternAttention(nn.Module):
    ...
    def forward(self, H, ht):
        ...
        new_alpha = alpha.view(-1, self.feat_size, 1).expand(
            -1, self.feat_size, self.filter_num
        )
